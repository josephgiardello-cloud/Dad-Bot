def test_should_evolve_persona_respects_session_gap(bot):
    ...
    bot.MEMORY_STORE["session_archive"] = [
        {
            "id": str(index),
            "summary": f"s{index}",
            "created_at": f"2026-04-{index + 1:02d}T10:00:00",
            "topics": ["general"],
    ...
    bot.MEMORY_STORE["persona_evolution"] = [
        {"trait": "more direct", "session_count": 3, "applied_at": "2026-04-01T10:00:00"}
    ]

    assert bot.long_term_signals.should_evolve_persona() is False

    bot.MEMORY_STORE["session_archive"].append(
        {
            "id": "6",
            "summary": "s6",
...
def test_evolve_persona_persists_entry_and_queues_announcement(bot, mocker):
    ...
    mocker.patch.object(
        ...
        side_effect=[
            {
                "message": {
                    "content": '{"new_trait": "gentler when you are hard on yourself", "reason": "You trust me more when you are self-critical."}'
                }
            },
            {
                "message": {
                    "content": '{"score": 9, "approved": true, "feedback": "Specific and steady.", "suggested_refinement": null}'
                }
            },
        ],
    )
    ...
    assert any(
        "little more gentler when you are hard on yourself" in message["message"]
        for message in bot.pending_proactive_messages()
    )
...
def test_evolve_persona_rejects_low_quality_trait_after_critique(bot, mocker):
    ...
    mocker.patch.object(
        ...
        side_effect=[
            ...
            {
                "message": {
                    "content": '{"score": 4, "approved": false, "feedback": "Too generic.", "suggested_refinement": null}'
                }
            },
        ],
    )
...
def test_reject_persona_trait_removes_latest_entry(bot):
    bot.MEMORY_STORE["persona_evolution"] = [
        ...
        },
        {
            ...
            "announcement": "",
            "session_count": 7,
            "applied_at": "2026-04-10T10:00:00",
        },
    ]
...
def test_generate_wisdom_insight_uses_mocked_ollama_and_deduplicates_recent_entries(bot, mocker):
    ...
    bot.MEMORY_STORE["consolidated_memories"] = [
        {
            "summary": "Tony has been carrying work stress while trying to stay steady.",
            "category": "work",
            "source_count": 2,
    ...
    mocker.patch.object(
        ...
        return_value={
            "message": {
                "content": '{"summary": "Tony does better when he gives himself a smaller next step instead of carrying the whole week at once.", "topic": "work"}'
            }
        },
    )
...
def test_detect_life_patterns_queues_proactive_message_for_new_pattern(bot):
    ...
    bot.MEMORY_STORE["session_archive"] = [
        ...
        {
            "summary": "Work feels heavy every Sunday.",
            "topics": ["work"],
            "dominant_mood": "stressed",
            "turn_count": 4,
