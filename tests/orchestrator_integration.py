...
class OrchestratorIntegrationLayer:
    ...
    def __init__(self, orchestrator: DadBotOrchestrator | None = None):
    ...
    def _capture_tool_trace(self, context: TurnContext) -> dict[str, Any]:
        ...
        tool_data["success_rate"] = successful / len(results) if results else 0.0
...
def print_orchestrator_integration_results(results: list[dict[str, Any]]) -> None:
    ...
    for result in results:
        if result["trace"]:
            ...
            print(f"\n  {scenario}:")
            print(f"    Planner: {planner.get('plan_length', 0)} steps, {planner.get('goal_count', 0)} goals")
            print(f"    Tools: {tools.get('execution_count', 0)} executed, {tools.get('success_rate', 0):.1%} success")
