...
for attr, _ in SUBSYSTEMS:
    ...
    print(f"  {attr:<28s}  {actual:<24s}  {flag}")
...
total = len(results)
print(f"  Scenarios run: {total}  Passed: {total_pass}  Failed: {total - total_pass}  Time: {t_b1 - t_b0:.2f}s")
print()
...
print(f"  Mock success rate: {pct:.1f}%")
...
for name, impl, proven in features:
    ...
    print(f"  {name:<38s}  {impl:<40s}  {s}")

proven_count = sum(1 for *_, p in features if p)
print()
print(f"  {proven_count}/{len(features)} features provably present at runtime.")
...
for lane, marker, count, result in lanes:
    print(f"  {lane:<16s}  {marker:<22s}  {count:>6s}  {result}")
