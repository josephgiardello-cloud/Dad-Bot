...
def _allocate_agent_goals(*, runtime, event, user_text: str, attachments: list[dict], thread_state, intent_result):
    _ = (runtime, event, user_text, attachments, thread_state, intent_result)
    return []


def _multi_agent_plan_merge(*, runtime, event, user_text: str, attachments: list[dict], thread_state, intent_result):
...
def _generate_candidates(*, runtime, enable_multi_agent_substrate: bool, event, user_text: str, attachments: list[dict], thread_state, intent_result):
...
def _turn_phase_plan_result(*, turn_state: dict[str, Any], execution_result: dict[str, Any]) -> "PlanResult":
    ...
    enable_multi_agent_substrate = bool(event.payload.get("enable_multi_agent_substrate", False))
...
def _turn_phase_plan(*, turn_state: dict[str, Any], execution_result: dict[str, Any]) -> tuple:
    ...
    result = _turn_phase_plan_result(turn_state=turn_state, execution_result=execution_result)
    return (
...
class Planner:
    def build(self, turn_state: dict[str, Any], execution_result: dict[str, Any]) -> "PlanResult":
        return _turn_phase_plan_result(turn_state=turn_state, execution_result=execution_result)

    def build_legacy(self, turn_state: dict[str, Any], execution_result: dict[str, Any]) -> tuple:
        """LEGACY tuple output entrypoint (deprecated, rollback only)."""
        return _turn_phase_plan(turn_state=turn_state, execution_result=execution_result)
