...
class SQLiteAsyncPolicyPersistence:
    """Async sqlite-backed persistence adapter for policy state and history."""
    ...
    def _initialize_schema(self) -> None:
        ...
        try:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS dadbot_policy_kv (
            ...
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS dadbot_policy_list (
                    key TEXT NOT NULL,
            ...
            conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_dadbot_policy_list_key_seq
                ON dadbot_policy_list(key, seq)
                """,
            )
            conn.commit()
        finally:
    ...
    def _get_sync(self, key: str) -> str | None:
        ...
        try:
            row = conn.execute(
                "SELECT value FROM dadbot_policy_kv WHERE key = ?",
                (str(key),),
            ).fetchone()
    ...
    def _set_sync(self, key: str, value: str) -> None:
        ...
        try:
            conn.execute(
                """
                INSERT INTO dadbot_policy_kv(key, value)
                VALUES(?, ?)
    ...
    def _rpush_sync(self, key: str, value: str) -> None:
        ...
        try:
            conn.execute(
                "INSERT INTO dadbot_policy_list(key, value) VALUES(?, ?)",
                (str(key), str(value)),
            )
            conn.commit()
        finally:
    ...
    def _lrange_sync(self, key: str, start: int, end: int) -> list[str]:
        ...
        try:
            rows = conn.execute(
                "SELECT value FROM dadbot_policy_list WHERE key = ? ORDER BY seq ASC",
                (str(key),),
            ).fetchall()
