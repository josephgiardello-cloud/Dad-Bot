...
class ContractNode:
    ...
    validator_fn: Callable[[], list[str]] | None = field(default=None, repr=False)
...
class ContractPropagationMap:
    ...
    def _revalidate(self, contract_id: str) -> ContractValidationResult:
        ...
        if violations:
            ...
            logger.debug(
                "[Phase 4.2] Contract '%s' v%s revalidated — OK",
                contract_id,
                node.version,
            )
...
def build_dadbot_contract_map() -> ContractPropagationMap:
    ...
    cmap.register(
        ContractNode(
            ...
            description=("Graph integrity — save_node_single_execution + mutation_safety from CAPABILITY_CONTRACTS"),
            validator_fn=lambda: _validate_capability(
                CAPABILITY_CONTRACTS,
                ["save_node_single_execution", "mutation_safety"],
            ),
        ),
    )
    cmap.register(
...
def _validate_runtime_contract() -> list[str]:
    ...
    try:
        ...
        if not callable(getattr(_m, "AppRuntimeContract", None)) and not hasattr(
            _m,
            "AppRuntimeContract",
        ):
...
def _validate_capability(
    ...
    for key in required_keys:
        if key not in contracts:
            ...
            entry = contracts[key]
            if not entry.get("runtime_enforcement") and not entry.get(
                "test_enforcement",
            ):
                issues.append(
                    f"Capability contract '{key}' has neither runtime_enforcement nor test_enforcement enabled",
                )
    return issues
