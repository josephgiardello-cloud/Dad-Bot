...
def enforce_authoritative_truth_system(
    *, metadata: dict[str, Any] | None = None, state: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    legacy_replay = bool(
        current_metadata.get("use_legacy_replay_engine")
        or current_state.get("use_legacy_replay_engine")
    )
    alternate_memory_truth = str(
        current_metadata.get("memory_truth_system")
        or current_state.get("memory_truth_system")
        or ""
    ).strip()
    violations: list[str] = []
