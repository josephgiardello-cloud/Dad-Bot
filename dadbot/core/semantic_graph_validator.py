...
class GraphSemanticValidator:
    ...
    def get_contract(self, node_name: str) -> Optional[NodeIntentContract]:
    ...
    def validate_graph(
        ...
        if violations:
            logger.warning(
                ...
                len(violations), list(node_order),
            )
        else:
            logger.debug(
...
def build_dadbot_semantic_validator() -> GraphSemanticValidator:
    ...
    validator.register_node_contract(NodeIntentContract(
        ...
        mutation_constraints=["MutationQueue.drain_only", "save_node_single_execution"],
