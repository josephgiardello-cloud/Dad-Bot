...
class RuntimeOrchestrationManager:
    ...
    def _normalize_background_task_record(self, payload):
        ...
        return BackgroundTaskRecord.model_validate(normalized_payload).model_dump(
            mode="python",
        )
            ...
            for task_id in self.bot._background_task_ids:
                ...
                if isinstance(payload, dict):
                    task_payloads.append(
                        self._normalize_background_task_record(payload),
                    )
    ...
    def submit_background_task(
        ...
        **kwargs,
        ...
        running_tasks = int(
            self.background_task_snapshot(limit=8).get("running", 0) or 0,
        )
