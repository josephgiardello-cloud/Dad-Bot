...
class StatusReportingManager:
    ...
    def graph_fallback_snapshot(self) -> dict:
        ...
        graph_issues = [item for item in issues if str(item.get("purpose") or "").strip().lower() == "turn_graph"]
        ...
        return GraphFallbackStatusSnapshot.model_validate(payload).model_dump(
            mode="python",
        )
    ...
    def circuit_breaker_snapshot(
        ...
        *,
        ...
        if health.get("clarification_recommended"):
            reasons.append(
                str(
                    health.get("clarification_message") or "Dad asked for a short clarification.",
                ).strip(),
            )
        if degraded_graph.get("active"):
            mode = str(degraded_graph.get("degraded_mode") or "legacy").replace(
                "_",
                " ",
            )
        ...
        return CircuitBreakerStatusSnapshot.model_validate(
            {
                ...
                "reasoning_confidence": float(
                    health.get("reasoning_confidence", 1.0) or 1.0,
                ),
            },
    ...
    def service_status_snapshot(self) -> dict:
        ...
        snapshot = {
            ...
            "event_stream_url": client.session_event_stream_url(
                self.bot.runtime_state_container.session_id,
                tenant_id=self.bot.TENANT_ID,
            ),
            "reachable": False,
            "port_open": False,
    ...
    def dashboard_status_snapshot(self) -> dict:
        ...
        history_messages = len(
            [message for message in self.bot.history if message.get("role") in {"user", "assistant"}],
        )
        top_topics = []
        ...
        prompt_guard = PromptGuardStatusSnapshot.model_validate(
            self.bot.prompt_guard_stats(),
        ).model_dump(mode="python")
        health = RuntimeHealthSnapshot.model_validate(
        ...
        validated = DashboardStatusSnapshot.model_validate(
            {
                ...
                "persistence": PersistenceStatusSnapshot.model_validate(
                    persistence,
                ).model_dump(mode="python"),
                "moderation": ModerationSnapshot.model_validate(moderation).model_dump(
                    mode="python",
                ),
                "background_tasks": BackgroundTaskOverview.model_validate(
                    self.bot.background_task_snapshot(),
                ).model_dump(mode="python"),
                "semantic_memory": dict(semantic_status or {}),
                "vision": VisionStatusSnapshot.model_validate(
                    {"ready": vision_ready, "message": vision_message},
                ).model_dump(mode="python"),
                ...
                "security": SecurityStatusSnapshot.model_validate(
                    {
                        ...
                        "has_pin_hint": bool(
                            str(security.get("pin_hint") or "").strip(),
                        ),
                    },
                ).model_dump(mode="python"),
                ...
                "active_thread": ActiveThreadSnapshot.model_validate(
                    {
                        ...
                        "last_message": str(
                            active_thread.get("last_message") or "",
                        ).strip(),
                    },
                ).model_dump(mode="python"),
                "relationship": RelationshipStatusSnapshot.model_validate(
                    {
                        ...
                        "openness_level": int(
                            relationship.get("openness_level", 0) or 0,
                        ),
                        "emotional_momentum": str(
                            relationship.get("emotional_momentum") or "steady",
                        ).strip()
                        or "steady",
                        "active_hypothesis": str(
                            relationship.get("active_hypothesis") or "supportive_baseline",
                        ).strip()
                        or "supportive_baseline",
                        "active_hypothesis_label": str(
                            relationship.get("active_hypothesis_label") or "Supportive Baseline",
                        ).strip()
                        or "Supportive Baseline",
                        "active_hypothesis_probability": float(
                            relationship.get("active_hypothesis_probability", 0.0) or 0.0,
                        ),
    ...
    def ui_shell_snapshot(self) -> dict:
        ...
        persona_traits = [
            entry.get("trait", "") for entry in self.bot.active_persona_trait_entries(limit=4) if entry.get("trait")
        ]
        return {
            "ollama": {
