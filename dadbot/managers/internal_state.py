...
class InternalStateManager:
    ...
    def snapshot(self):
        ...
        normalized["proactive_recommended"] = bool(
            raw.get("proactive_recommended", False),
        )
        ...
        normalized["recent_reflections"] = [
            str(item).strip() for item in list(raw.get("recent_reflections") or [])[-8:] if str(item).strip()
        ]
        history = []
                ...
                history.append(
                    {
                        ...
                        "support": self._clamp_unit(
                            item.get("support", 0.0),
                            fallback=0.0,
                        ),
                        "challenge": self._clamp_unit(
                            item.get("challenge", 0.0),
                            fallback=0.0,
                        ),
                        "proactive": self._clamp_unit(
                            item.get("proactive", 0.0),
                            fallback=0.0,
                        ),
                    },
                )
        ...
        for bucket_name in ("belief_vector", "goal_vector", "tone_targets"):
            normalized[bucket_name] = {
                key: self._clamp_unit(
                    value,
                    fallback=defaults[bucket_name].get(key, 0.5),
                )
    ...
    def reflect_after_turn(self, user_input, current_mood, dad_reply=""):
        ...
        beliefs["stability"] = self._clamp_unit(
            self._blend(beliefs.get("stability", 0.5), target["stability"], alpha=0.2),
        )
        beliefs["stress_load"] = self._clamp_unit(
            self._blend(
                beliefs.get("stress_load", 0.3),
                target["stress_load"],
                alpha=0.2,
            ),
        )
        beliefs["connection"] = self._clamp_unit(
            self._blend(
                beliefs.get("connection", 0.5),
                target["connection"],
                alpha=0.16,
            ),
        )
        ...
        goals["support"] = self._clamp_unit(
            self._blend(
                goals.get("support", 0.7),
                0.82 if mood in {"sad", "stressed", "tired"} else 0.68,
                alpha=0.2,
            ),
        )
        goals["challenge"] = self._clamp_unit(
            self._blend(
                goals.get("challenge", 0.25),
                0.2 if mood in {"sad", "stressed", "tired"} else 0.42,
                alpha=0.18,
            ),
        )
        goals["proactive"] = self._clamp_unit(
            self._blend(
                goals.get("proactive", 0.35),
                0.56 if mood in {"sad", "stressed", "tired"} else 0.32,
                alpha=0.16,
            ),
        )

        tone = dict(state.get("tone_targets") or {})
        tone["warmth"] = self._clamp_unit(
            self._blend(tone.get("warmth", 0.7), target["warmth"], alpha=0.22),
        )
        tone["directness"] = self._clamp_unit(
            self._blend(tone.get("directness", 0.45), target["directness"], alpha=0.22),
        )
        tone["playfulness"] = self._clamp_unit(
            self._blend(tone.get("playfulness", 0.3), target["playfulness"], alpha=0.22),
        )

        proactive_recommended = bool(
            goals.get("proactive", 0.0) >= 0.5 or beliefs.get("stress_load", 0.0) >= 0.7,
        )
