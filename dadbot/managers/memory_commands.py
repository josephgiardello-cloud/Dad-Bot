...
class MemoryCommandManager:
    ...
    def parse_memory_command(user_input):
        ...
        update_match = re.match(
            r"^(?:update memory|change memory)(?: that)? (.+?) to (.+)$",
            stripped,
            flags=re.IGNORECASE,
        )
        if update_match:
        ...
        remember_match = re.match(
            r"^(?:remember this|remember that|please remember)(?::)?\s+(.+)$",
            stripped,
            flags=re.IGNORECASE,
        )
        if remember_match:
        ...
        forget_match = re.match(
            r"^(?:forget|delete memory about|remove memory about|forget that)(?::)?\s+(.+)$",
            stripped,
            flags=re.IGNORECASE,
        )
        if forget_match:
        ...
        if re.match(
            r"^/(?:delete\s+my\s+data|forget\s+me|erase\s+me)$",
            stripped,
            flags=re.IGNORECASE,
        ):
    ...
    def handle_memory_command(self, user_input):
        ...
        if command["action"] == "remember":
            ...
            return (
                f"I've got it, Tony. I'll remember that under {memory['category']} as: {memory['summary'].rstrip('.')}"
            )
    ...
    def _handle_data_export(self) -> str:
        ...
        try:
            ...
            consolidated = list(
                getattr(self.bot, "consolidated_memories", list)() or [],
            )
            relationship = {}
            try:
            ...
            counts = f"{len(memories)} memories, {len(consolidated)} consolidated entries"
            return (
        ...
        try:
            before_snapshot = {
                ...
                "consolidated_count": len(
                    list(getattr(self.bot, "consolidated_memories", list)() or []),
                ),
            }
            ...
            if bool(getattr(config, "rtbf_proof_enabled", True)) and session_log_dir is not None:
                ...
                receipt_path = Path(session_log_dir) / "rtbf_receipts.jsonl"
