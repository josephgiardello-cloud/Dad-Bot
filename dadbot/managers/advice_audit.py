        ...
        *,
        ...
        try:
            ...
            active_hypothesis = str(
                rel.get("active_hypothesis") or "supportive_baseline",
            )
        except Exception:
            pass
        ...
        hard_tone = self._has_hard_tone(reply)
        needs_repair = bool(
            alignment < 45 or (hard_tone and self.bot.normalize_mood(current_mood) in {"sad", "stressed", "tired"}),
        )
        ...
        audit_entry = {
            ...
            "goal_vector": {
                ...
                "challenge": round(
                    float(goal_vector.get("challenge", 0.25) or 0.25),
                    3,
                ),
                "proactive": round(
                    float(goal_vector.get("proactive", 0.35) or 0.35),
                    3,
                ),
            },
        ...
        audits = [dict(item) for item in list(store.get("advice_audits") or []) if isinstance(item, dict)]
