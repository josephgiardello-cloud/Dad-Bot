...
class ReplySupervisorManager:
    ...
    def _normalize_supervisor_issues(issues: object) -> list[str]:
        if not isinstance(issues, list):
            return []
        return [str(item).strip() for item in issues if str(item).strip()][:4]
    ...
    def _last_supervisor_duration_ms(self) -> int:
        return max(
            0,
            int((self.bot._last_reply_supervisor or {}).get("duration_ms", 0) or 0),
        )

    def _coerce_supervisor_judgment(
        self,
        judgment,
        *,
        stage: str,
    ) -> SupervisorJudgment | None:
        ...
        payload = {
            ...
            "dad_likeness": self._coerce_supervisor_rating(
                judgment.get("dad_likeness"),
            ),
            "groundedness": self._coerce_supervisor_rating(
                judgment.get("groundedness"),
            ),
            "emotional_fit": self._coerce_supervisor_rating(
                judgment.get("emotional_fit"),
            ),
    ...
    def _supervisor_messages(
        ...
        return [
            {
                "role": "user",
                "content": self.build_reply_supervisor_prompt(
                    user_input,
                    candidate_reply,
                    current_mood,
                ),
    ...
    def _finalize_supervisor_response(
        ...
        *,
        ...
        return self.apply_reply_supervisor_decision(
            judgment,
            candidate_reply,
            stage=stage,
        )

    def build_reply_critique_prompt(
        self,
        user_input: str,
        draft_reply: str,
        current_mood: str,
...
""".strip()
    ...
    def build_reply_alignment_judge_prompt(
        self,
        user_input: str,
        candidate_reply: str,
        current_mood: str,
    ) -> str:
        return self.build_reply_supervisor_prompt(
            user_input,
            candidate_reply,
            current_mood,
        )

    def build_reply_supervisor_prompt(
        self,
        user_input: str,
        draft_reply: str,
        current_mood: str,
...
""".strip()
    ...
    def apply_reply_supervisor_decision(
        self,
        judgment,
        candidate_reply,
        stage="reply_supervisor",
    ):
...
""".strip()
    ...
    def reply_supervisor_snapshot(self):
        ...
        decision = self._supervisor_state(
            ...
            approved=bool(
                (self.bot._last_reply_supervisor or {}).get("approved", True),
            ),
            score=max(
                0,
                min(
                    10,
                    int((self.bot._last_reply_supervisor or {}).get("score", 0) or 0),
                ),
            ),
            dad_likeness=max(
                0,
                min(
                    10,
                    int(
                        (self.bot._last_reply_supervisor or {}).get("dad_likeness", 0) or 0,
                    ),
                ),
            ),
            groundedness=max(
                ...
                min(
                    10,
                    int(
                        (self.bot._last_reply_supervisor or {}).get("groundedness", 0) or 0,
                    ),
                ),
            ),
            emotional_fit=max(
                ...
                min(
                    10,
                    int(
                        (self.bot._last_reply_supervisor or {}).get("emotional_fit", 0) or 0,
                    ),
                ),
            ),
            issues=[
                str(item) for item in (self.bot._last_reply_supervisor or {}).get("issues", []) if str(item).strip()
            ][:4],
    ...
    def judge_reply_alignment(self, user_input, candidate_reply, current_mood):
        return self.run_reply_supervisor(
            user_input,
            candidate_reply,
            current_mood,
    ...
    async def judge_reply_alignment_async(
        self,
        user_input,
        candidate_reply,
        current_mood,
    ):
        return await self.run_reply_supervisor_async(
            user_input,
            candidate_reply,
            current_mood,
            stage="alignment_judge",
