...
class SessionSummaryManager:
	...
	def build_session_summary_prompt(self, previous_summary: str | None, messages: list[dict[str, Any]]) -> str | None:
...
""".strip()
	...
	def refresh_session_summary(self, force: bool = False):
		...
		try:
			...
			self.bot.record_runtime_issue("session summarization", "keeping the previous rolling summary", exc)
			return self.bot.session_summary

		if summary:
			self.bot.session_summary = summary
			self.bot.session_summary_covered_messages = eligible_count
			self.bot.session_summary_updated_at = datetime.now().isoformat(timespec="seconds")
