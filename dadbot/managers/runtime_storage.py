...
class RuntimeStorageManager:
	...
	def save_profile(self):
		with self.bot._io_lock:
			if self.bot._tenant_document_store is not None:
				self.bot._tenant_document_store.save_session_state("profile", self.bot.PROFILE)
				return
			self.write_json_atomically(self.bot.PROFILE_PATH, self.bot.PROFILE, backup=True)
