...
class RuntimeModelManager:
    ...
    def ollama_show_payload(self, model_name=None):
        ...
        try:
            ...
            logger.debug(
                "Could not inspect Ollama model metadata for %s: %s",
                candidate,
    ...
    def normalized_llm_model(self, model_name: str | None = None) -> str:
        model = str(
            model_name or getattr(self.bot, "LLM_MODEL", "") or self.bot.MODEL_NAME,
        ).strip()
    ...
    def dedicated_embedding_model_candidates(self):
        candidates = []
        for model_name in [
            self.bot.ACTIVE_EMBEDDING_MODEL,
            *self.bot.PREFERRED_EMBEDDING_MODELS,
        ]:
