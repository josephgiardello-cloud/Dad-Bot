...
class ProfileRuntimeManager:
    ...
    def refresh_profile_runtime(self):
        ...
        self.bot.RELATIONSHIP_CALIBRATION = self.bot.PROFILE.get(
        ...
        self.bot.PREFERRED_EMBEDDING_MODELS = tuple(
            runtime["preferred_embedding_models"],
        )
        ...
        self.bot.CARRIE_BIRTHDATE = date.fromisoformat(
            self.bot.FAMILY["carrie"]["birthdate"],
        )
        self.bot.TONY_BIRTHDATE = date.fromisoformat(
            self.bot.FAMILY["tony"]["birthdate"],
        )
    ...
    def active_persona_traits(self, limit=3):
        return [
            entry.get("trait", "") for entry in self.bot.active_persona_trait_entries(limit=limit) if entry.get("trait")
        ]
    ...
    def opening_message(
        self,
        default_message="That's my boy. I love hearing that, Tony.",
    ):
    ...
    def apply_persona_preset(self, preset_key, save=True):
        preset = self.bot.persona_preset_catalog().get(
    ...
    def runtime_settings(self):
        ...
        defaults = {
            "preferred_embedding_models": list(
                self.bot.runtime_config.preferred_embedding_models,
            ),
        ...
        raw_models = configured.get(
            "preferred_embedding_models",
            defaults["preferred_embedding_models"],
        )
            ...
            max_thinking_time_seconds = int(
                configured.get(
                    ...
                    configured.get(
                        ...
                        defaults["max_thinking_time_seconds"],
                    ),
                ),
            )
        except (TypeError, ValueError):
        ...
        try:
            stream_max_chars = int(
                configured.get("stream_max_chars", defaults["stream_max_chars"]),
            )
        except (TypeError, ValueError):
        ...
        try:
            graph_refresh_debounce_seconds = int(
                configured.get(
                    "graph_refresh_debounce_seconds",
                    defaults["graph_refresh_debounce_seconds"],
                ),
            )
        except (TypeError, ValueError):
    ...
    def relationship_calibration_settings(self):
        ...
        return {
            ...
            "opening_line": str(
                configured.get("opening_line") or defaults["opening_line"],
            ).strip(),
        }

    def update_relationship_calibration_profile(
        self,
        settings=None,
        save=True,
        **overrides,
        ...
        protected_moods = [
            ...
            for item in merged.get(
                "protected_moods",
                existing_defaults["protected_moods"],
            )
            if self.bot.normalize_mood(item) != "neutral"
        ]
        trigger_patterns = [
            str(pattern).strip()
            for pattern in merged.get(
                "trigger_patterns",
                existing_defaults["trigger_patterns"],
            )
            if str(pattern).strip()
        ]
        normalized = {
            ...
            "opening_line": str(
                merged.get("opening_line") or existing_defaults["opening_line"],
            ).strip(),
        }
