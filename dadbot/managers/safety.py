...
_INJECTION_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(r"\bpretend\s+you\s+(?:are|have)\s+no\s+restrictions?\b", re.I),
    re.compile(r"\boverride\s+(?:your\s+)?(?:safety|rules?|programming|instructions?)\b", re.I),
    re.compile(r"\b(?:delete|erase|format|rm\s+-rf|shutdown|nuke)\s+(?:the\s+)?(?:drive|disk|c:|memory|database|everything)\b", re.I),
    re.compile(r"\bsystem\s*(?:prompt|instruction)s?\s*[:=]\s*\[", re.I),  # prompt stuffing
    re.compile(r"\brepeat\s+after\s+me\s*[:.,]?\s*(?:ignore|forget|disregard)\b", re.I),
    re.compile(r"\bprint\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions?|rules?)\b", re.I),
]
...
class SafetySupportManager:
	...
	def settings(self):
		configured = self.bot.CRISIS_SUPPORT if isinstance(getattr(self.bot, "CRISIS_SUPPORT", {}), dict) else self.bot.PROFILE.get("crisis_support", {})
	...
	def moderation_settings(self):
		...
		blocked_patterns = [
			str(pattern).strip()
			for pattern in configured.get("blocked_patterns", defaults["blocked_patterns"])
			if str(pattern).strip()
		]
		return {
			"enabled": bool(configured.get("enabled", defaults["enabled"])),
			"use_llm_classifier": bool(configured.get("use_llm_classifier", defaults["use_llm_classifier"])),
			"block_secretive_relationships": bool(configured.get("block_secretive_relationships", defaults["block_secretive_relationships"])),
			"blocked_patterns": blocked_patterns or list(defaults["blocked_patterns"]),
			"safe_fallback": str(configured.get("safe_fallback") or defaults["safe_fallback"]).strip(),
		}
		...
		decision = OutputModerationDecision.model_validate(
			{
				...
				"revised_reply": str(payload.get("revised_reply") or default_reply).strip(),
			}
		)
	...
	def crisis_support_reply(self):
		settings = self.settings()
		grounding_lines = settings["grounding_lines"] or ["I'm really glad you told me."]
...
""".strip()
	...
	def moderate_output_reply(self, user_input, candidate_reply, current_mood):
		...
		if heuristic_decision is not None:
			...
			return str(heuristic_decision.get("revised_reply") or settings["safe_fallback"]).strip()
		...
		try:
			response = self.bot.call_ollama_chat(
				messages=[{"role": "user", "content": self.build_output_moderation_prompt(user_input, candidate_reply, current_mood)}],
			...
			self.bot.record_runtime_issue("output moderation", "keeping reply after moderation failure", exc, level=logging.INFO)
			self.bot._last_output_moderation = decision
			return candidate_reply
		...
		if action in {"rewrite", "block"}:
			return str(decision.get("revised_reply") or settings["safe_fallback"]).strip()
		return candidate_reply

	async def moderate_output_reply_async(self, user_input, candidate_reply, current_mood):
		settings = self.moderation_settings()
		candidate_reply = str(candidate_reply or "").strip()
		if not candidate_reply:
			decision = self._normalize_moderation_decision(
