...
class LongTermSignalsManager:
	...
	def decayed_trait_strength(self, entry):
		...
		anchor = str(entry.get("last_reinforced_at") or entry.get("applied_at") or "").strip()
	...
	def trait_priority(self, entry):
		return round(
			self.decayed_trait_strength(entry)
			+ self.trait_impact(entry) * 0.35
			+ self.critique_score(entry) * 0.05,
			4,
		)
	...
	def most_positive_persona_traits(self, limit=3):
		positive_entries = [entry for entry in self.active_persona_trait_entries(limit=max(3, limit or 3) * 2) if self.trait_impact(entry) > 0]
	...
	def recent_mood_trend(self):
		if not self.bot.session_moods:
			return "neutral"
		return Counter([mood for mood in self.bot.session_moods[-10:] if mood]).most_common(1)[0][0]
...
"""
		try:
			...
			return {"score": 5, "approved": True, "feedback": "critique failed", "suggested_refinement": None}
	...
	def consolidate_persona_evolution_history(self, history=None, persist=False):
		...
		merged = []
		for entry in ranked_history:
			match = next((existing for existing in merged if self.trait_similarity(existing.get("trait", ""), entry.get("trait", "")) >= 0.72), None)
			if match is None:
			...
			if len(str(entry.get("critique_feedback", ""))) > len(str(match.get("critique_feedback", ""))):
				match["critique_feedback"] = entry.get("critique_feedback", "")
			match["session_count"] = max(int(match.get("session_count", 0)), int(entry.get("session_count", 0)))
			match["strength"] = round(min(3.0, max(self.trait_strength(match), self.trait_strength(entry)) + 0.1), 2)
			match["impact_score"] = round(max(self.trait_impact(match), self.trait_impact(entry)), 2)
			match["critique_score"] = max(self.critique_score(match), self.critique_score(entry))
			match["applied_at"] = max(str(match.get("applied_at", "")), str(entry.get("applied_at", "")))
			match["last_reinforced_at"] = max(str(match.get("last_reinforced_at", "")), str(entry.get("last_reinforced_at", "")))

		max_traits = min(8, int(self.bot.runtime_config.store_limits.get("persona_evolution", 8)))
		trimmed = sorted(
			merged,
			key=lambda entry: (
				-self.trait_priority(entry),
	...
	def evolve_persona(self, force=False):
		...
		try:
			response = self.bot.call_ollama_chat(
				messages=[{"role": "user", "content": self.build_persona_evolution_prompt()}],
		...
		if any(self.trait_similarity(existing.get("trait", ""), trait) >= 0.72 for existing in self.bot.persona_evolution_history()):
			return None
		...
		if entry.get("announcement"):
			self.bot.queue_proactive_message(entry["announcement"], source="persona-evolution")
		return entry
	...
	def reject_persona_trait(self, trait_query=""):
		...
		if not query or query.lower() == "trait":
			...
			for index in range(len(history) - 1, -1, -1):
				...
				if normalized_query in self.bot.normalize_memory_text(trait) or self.trait_similarity(trait, query) >= 0.72:
					target_index = index
					break
		...
		self.bot.mutate_memory_store(persona_evolution=self.consolidate_persona_evolution_history(history))
		self.bot.mark_memory_graph_dirty()
		return removed

	def update_trait_impact_from_relationship_feedback(self, trust_delta, openness_delta):
		history = [dict(entry) for entry in self.bot.persona_evolution_history()]
		if not history:
			return []
		...
		latest_entry = max(refreshed, key=lambda entry: str(entry.get("applied_at", "")))
		latest_entry["impact_score"] = round(self.trait_impact(latest_entry) + total_delta, 2)
		if total_delta > 0:
			latest_entry["strength"] = round(min(3.0, self.trait_strength(latest_entry) + 0.12 * total_delta), 2)
			latest_entry["last_reinforced_at"] = datetime.now().isoformat(timespec="seconds")
		elif total_delta < 0:
			latest_entry["strength"] = round(max(0.25, self.trait_strength(latest_entry) + 0.08 * total_delta), 2)
	...
	def add_memory_nodes_to_graph(self, node_weights, node_types, edge_weights):
		for memory in self.bot.MEMORY_STORE.get("memories", []):
			...
			weight = self.bot.recency_weight(memory.get("updated_at") or memory.get("created_at"))
			if category and category != "general":
				self.accumulate_memory_graph_node(node_weights, node_types, category, "category", weight)
			if mood != "neutral":
				self.accumulate_memory_graph_node(node_weights, node_types, mood, "mood", weight)
	...
	def add_relationship_topics_to_graph(self, node_weights, node_types):
		for topic, count in self.bot.relationship_state().get("recurring_topics", {}).items():
			...
			if topic_name and topic_name != "general":
				self.accumulate_memory_graph_node(node_weights, node_types, topic_name, "topic", max(1, int(count)))
	...
	def add_archive_nodes_to_graph(self, node_weights, node_types, edge_weights):
		for entry in self.bot.session_archive():
			...
			for topic in entry.get("topics", []):
				self.accumulate_memory_graph_node(node_weights, node_types, topic, "topic", weight)
				if mood != "neutral":
	...
	def build_memory_graph_nodes(node_weights, node_types):
		return [
			...
			for label, weight in sorted(node_weights.items(), key=lambda item: (-item[1], item[0]))[:18]
		]
	...
	def refresh_memory_graph(self, force=False):
		...
		if not force:
			...
			debounce_seconds = max(0, int(getattr(self.bot, "GRAPH_REFRESH_DEBOUNCE_SECONDS", 0) or 0))
			last_refresh = float(getattr(self.bot, "_last_memory_graph_refresh_monotonic", 0.0) or 0.0)
	...
	def should_generate_wisdom_insight(self, user_input, force=False):
		...
		return self.bot.session_turn_count() % turn_interval == 0 or bool(self.bot.significant_tokens(user_input) & set(self.bot.relationship.top_topics()))

	def build_wisdom_prompt(self, user_input):
		consolidated_lines = "\n".join(
			f"- {entry.get('summary', '')}"
			for entry in self.bot.consolidated_memories()[-5:]
		) or "- None yet."
		return DadPrompts.wisdom_prompt(self.summarize_memory_graph(), consolidated_lines, user_input)
	...
	def generate_wisdom_insight(self, user_input, force=False):
		...
		try:
			response = self.bot.call_ollama_chat(
				messages=[{"role": "user", "content": self.build_wisdom_prompt(user_input)}],
			...
			self.bot.record_runtime_issue("wisdom insight", "skipping wisdom generation for this turn", exc)
			return None
		...
		if any(self.bot.normalize_memory_text(item.get("summary", "")) == self.bot.normalize_memory_text(entry["summary"]) for item in existing[-recent_dedup_window:]):
			return next((item for item in reversed(existing) if self.bot.normalize_memory_text(item.get("summary", "")) == self.bot.normalize_memory_text(entry["summary"])), None)

		existing.append(entry)
		self.bot.mutate_memory_store(wisdom_insights=self.bot.runtime_config.tail(existing, "wisdom_insights"))
		return entry

	def relevant_wisdom_for_input(self, user_input):
		scored = []
		for entry in self.bot.wisdom_catalog():
			score = self.bot.memory_relevance_score(user_input, f"{entry.get('topic', '')} {entry.get('summary', '')}")
			if score > 0:
	...
	def _derived_deep_pattern_documents(self):
		archive_window = self.bot.runtime_config.window("deep_pattern_archive_window", 18)
		...
		min_occurrences = max(2, int(self.bot.cadence_settings().get("deep_pattern_min_occurrences", 3) or 3))
		documents = []
	...
	def deep_pattern_documents(self):
			...
			documents.append(
				{
					...
					"strength": round(max(0.2, min(1.0, int(pattern.get("confidence", 0) or 0) / 100.0)), 2),
				}
			)
			...
			documents.append(
				{
					...
					"strength": round(max(0.25, min(1.0, float(entry.get("confidence", 0.5) or 0.5))), 2),
				}
			)
		...
		for insight in list(self.bot.MEMORY_STORE.get("longitudinal_insights") or []):
			if not isinstance(insight, dict):
				continue
			summary = str(insight.get("insight") or insight.get("summary") or "").strip()
			if not summary:
				continue
			...
			documents.append(
				{
					...
					"mood": self.bot.normalize_mood(insight.get("dominant_mood") or "neutral"),
					"summary": summary,
					"evidence": str(insight.get("evidence") or "").strip(),
					"updated_at": str(insight.get("updated_at") or insight.get("created_at") or "").strip(),
	...
	def synthesize_longitudinal_insights(self, force=False, reference_time=None, max_items=12):
		now = self.bot.maintenance_scheduler._coerce_reference_time(reference_time) if hasattr(self.bot, "maintenance_scheduler") else datetime.now().replace(second=0, microsecond=0)
		last_run = str(self.bot.MEMORY_STORE.get("last_insight_synthesis_at") or "").strip()
		...
		insights = []
		for (topic, mood), payload in sorted(clusters.items(), key=lambda item: item[1].get("count", 0), reverse=True):
	...
	def deep_pattern_matches(self, user_input, limit=3):
		...
		candidate_limit = max(limit, self.bot.runtime_config.window("deep_pattern_candidates", 14))
		scored = []
		seen = set()
		for document in self.deep_pattern_documents():
			...
			age_days = self.bot.days_since_iso_date(document.get("updated_at") or document.get("created_at"))
	...
	def build_deep_pattern_context(self, user_input, limit=None):
		matches = self.deep_pattern_matches(
			user_input,
			limit=max(1, int(limit or self.bot.runtime_config.window("deep_pattern_context_limit", 3) or 3)),
		)
		if not matches:
			return None
		...
		for document in matches:
			source_label = source_labels.get(document.get("source_type"), "Long-term signal")
	...
	def detect_life_patterns(self, force=False):
		...
		for entry in archive[-cadence["life_pattern_window"]:]:
			...
			for topic in entry.get("topics", []) or ["general"]:
				key = (created_dt.strftime("%A"), str(topic).strip().lower() or "general", mood)
				combinations[key] = combinations.get(key, 0) + 1

		detected = []
		existing_summaries = {self.bot.normalize_memory_text(item.get("summary", "")) for item in self.bot.life_patterns()}
		existing_pattern_keys = {
			key
			for key in (self.pattern_identity(item) for item in self.bot.life_patterns())
			if key is not None
		}
		for (day_name, topic, mood), count in sorted(combinations.items(), key=lambda item: (-item[1], item[0])):
			if count < cadence["life_pattern_min_occurrences"] or topic == "general":
				continue

			confidence = min(95, 40 + count * 15 + (10 if mood in {"stressed", "sad", "frustrated", "tired"} else 0))
			...
			existing_summaries.add(self.bot.normalize_memory_text(pattern.get("summary", "")))
		...
		if detected:
			...
			for pattern in detected[:cadence["life_pattern_queue_limit"]]:
				self.bot.queue_proactive_message(self.build_pattern_message(pattern), source="life-pattern")
			return detected
	...
	def family_echo(self, user_input, current_mood):
		...
		try:
			response = self.bot.call_ollama_chat(
				messages=[{"role": "user", "content": self.build_family_echo_prompt(user_input, current_mood)}],
				options={"temperature": 0.3},
				purpose="family echo",
			)
	...
	def _aggregate_learning_signals(self) -> dict:
		...
		return {
			"relationship_feedback_count": len((self.bot.MEMORY_STORE.get("relationship_history") or [])[-10:]),
			"consolidated_votes": len([m for m in (self.bot.consolidated_memories() or []) if m.get("feedback_score")]),
	...
	def perform_continuous_learning_cycle(self) -> dict:
		...
		_apply_feedback = getattr(self.bot.memory_coordinator, "apply_consolidated_feedback_from_recent_turns", None)
		...
		self.bot.mutate_memory_store(
			...
			learning_cycle_count=int(self.bot.MEMORY_STORE.get("learning_cycle_count", 0) or 0) + 1,
			save=True,
		)
		...
		self.bot.runtime_state_container.record_state_update("continuous_learning_completed", result)
		return result
