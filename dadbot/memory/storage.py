...
from dadbot.core.execution_boundary import MemoryWriteOwnerScope, enforce_memory_write_owner
from dadbot.core.execution_trace_context import record_execution_step
from dadbot.core.transaction_coordinator import TransactionContext, TransactionCoordinator
...
def _section_changed(before: dict, after: dict, keys: set[str] | frozenset[str]) -> bool:
...
class _SemanticIndexParticipant:
    def __init__(self, backend: "MemoryStorageBackend", before_store: dict, after_store: dict) -> None:
        self._backend = backend
...
class MemoryStorageBackend:
    ...
    def _write_json_memory_store_unlocked(self):
        try:
            self.bot.write_json_atomically(self.bot.MEMORY_PATH, self._manager.memory_store, backup=True)
    ...
    def _write_memory_store_unlocked(self):
        if getattr(self.bot, "_tenant_document_store", None) is not None:
            try:
                self.bot._tenant_document_store.save_session_state("memory", self._manager.memory_store)
            except OSError as exc:
                self.bot.record_runtime_issue(
                    "memory store save",
    ...
    def _apply_mutation_in_memory(self, *, mutator=None, changes=None, normalize=True):
        ...
        if normalize:
            self._manager.memory_store = self._manager.normalize_memory_store(self._manager.memory_store)
    ...
    def _coordinated_commit(self, *, before_store: dict, changes: dict) -> None:
        ...
        semantic_changed = bool(_SEMANTIC_TRACKED_KEYS.intersection(changed_keys)) or _section_changed(
            before_store,
            after_store,
            _SEMANTIC_TRACKED_KEYS,
        )
        graph_changed = bool(_GRAPH_TRACKED_KEYS.intersection(changed_keys)) or _section_changed(
            before_store,
            after_store,
            _GRAPH_TRACKED_KEYS,
        )
        ...
        if semantic_changed:
            coordinator.register(_SemanticIndexParticipant(self, before_store, after_store))
        if graph_changed:
        ...
        if not report.ok:
            raise RuntimeError(
                "Cross-store transaction failed and was rolled back: "
                f"{report.error or 'unknown error'}"
            )

    def mutate_memory_store(self, mutator=None, normalize=True, save=True, owner=None, **changes):
        ...
        if io_lock is None:
            ...
            self._apply_mutation_in_memory(mutator=mutator, changes=changes, normalize=normalize)
            if save:
        ...
        if tenant_document_store is not None:
            ...
            if isinstance(persisted, dict):
                ...
                if normalized != persisted:
                    ...
                    self.bot.write_json_atomically(self.bot.MEMORY_PATH, normalized, backup=False)
                return normalized
    ...
    def export_memory_store(self, export_path):
        ...
        with self.bot._io_lock:
            normalized = self._manager.normalize_memory_store(self._manager.memory_store)
