...
class MemoryGraphManager:
    ...
    def _build_graph_store_backend(self):
        ...
        if postgres_dsn:
            try:
                ...
                logger.warning("Postgres graph store unavailable, falling back to SQLite: %s", exc)
    ...
    def graph_edge_key(source_key, target_key, relation_type):
        return hashlib.sha1(f"{source_key}|{relation_type}|{target_key}".encode("utf-8")).hexdigest()[:20]
    ...
    def graph_source_id(self, source_type, entry):
        if source_type == "archive_session":
            return str(entry.get("id") or hashlib.sha1(f"{entry.get('summary', '')}|{entry.get('created_at', '')}".encode("utf-8")).hexdigest()[:12])
        if source_type == "persona_trait":
            return hashlib.sha1(f"{entry.get('trait', '')}|{entry.get('applied_at', '')}".encode("utf-8")).hexdigest()[:12]
        if source_type == "life_pattern":
            return hashlib.sha1(
                f"{entry.get('summary', '')}|{entry.get('topic', '')}|{entry.get('day_hint', '')}|{entry.get('mood', '')}".encode("utf-8")
            ).hexdigest()[:12]
        return hashlib.sha1(f"{entry.get('summary', '')}|{entry.get('updated_at', '')}".encode("utf-8")).hexdigest()[:12]

    def graph_keyword_tokens(self, text, limit=4):
        ignored = {
            "carry", "carries", "dad", "lately", "often", "really", "shared", "talk", "talking", "tony",
            "want", "wants", "with", "your", "been", "just", "more", "about", "again", "this", "that",
        }
        ordered = []
        seen = set()
    ...
    def relation_type_for_entity(self, source_type, entity_type, text, fallback="mentions"):
        return self._entity_resolver.relation_type_for_entity(source_type, entity_type, text, fallback)

    # ------------------------------------------------------------------
    # Fact extraction
    # ------------------------------------------------------------------

    def extract_typed_graph_facts(self, text, source_type, *, default_category="general", default_mood="neutral", default_day=""):
        summary = str(text or "").strip()
        lowered = summary.lower()
        facts = []
        seen = set()

        def add_fact(label, semantic_type, relation_type=None, weight=1.0, confidence=0.7):
            entity_type, canonical = self.canonical_graph_entity(label, semantic_type=semantic_type)
            if not canonical:
                return
            resolved_relation = self.relation_type_for_entity(source_type, entity_type, lowered, fallback=relation_type or "mentions")
        ...
        if default_category and default_category != "general":
            add_fact(default_category, "category", relation_type="concerns", weight=1.1, confidence=0.8)
        if default_mood and default_mood != "neutral":
            add_fact(default_mood, "emotion", relation_type="feels", weight=1.0, confidence=0.82)
        if default_day:
            add_fact(default_day, "day", relation_type="mentioned_on", weight=0.9, confidence=0.78)
        ...
        for canonical, aliases in self.GRAPH_ENTITY_ALIASES.items():
            if lowered == canonical or canonical in lowered or any(alias in lowered for alias in aliases):
                add_fact(canonical, self.GRAPH_ENTITY_TYPES.get(canonical, "topic"), weight=1.1, confidence=0.8)

        if any(token in lowered for token in ["proud", "relieved", "grateful"]):
            add_fact("positive", "emotion", relation_type="feels", weight=0.95, confidence=0.78)
        if any(token in lowered for token in ["stress", "stressed", "overwhelmed", "anxious", "worried", "heavy", "pressure"]):
            if default_category and default_category != "general":
                add_fact(f"{default_category}_pressure", "stressor", relation_type="struggles_with", weight=1.15, confidence=0.82)
            else:
                add_fact("stress", "stressor", relation_type="struggles_with", weight=1.0, confidence=0.78)

        llm_facts = self.llm_typed_graph_facts(summary, source_type)
        for fact in llm_facts:
            entity_type, canonical = self.canonical_graph_entity(fact.get("object") or fact.get("label"), semantic_type=fact.get("object_type") or fact.get("entity_type") or "topic")
            if not canonical:
                continue
            relation_type = self.normalize_relation_type(fact.get("predicate") or fact.get("relation_type") or "mentions")
            ...
            facts.append({
                ...
                "confidence": max(0.3, min(0.95, float(fact.get("confidence", 0.7) or 0.7))),
            })

        for token in self.graph_keyword_tokens(summary, limit=3):
            add_fact(token, "topic", relation_type="mentions", weight=0.7, confidence=0.65)
        return facts
...
""".strip()
    ...
    def graph_source_confidence(self, source_type, entry):
        if source_type == "consolidated_memory":
            return round(self._bot.normalize_confidence(
        ...
        if source_type == "archive_session":
            try:
                ...
                turn_count = 1
            numeric = min(0.92, 0.5 + min(turn_count, 8) * 0.04 + 0.04 * len(entry.get("topics", [])))
        ...
        if source_type == "life_pattern":
            try:
                pattern_confidence = max(0, min(100, int(entry.get("confidence", 0) or 0)))
            except (TypeError, ValueError):
    ...
    def upsert_graph_node(node_map, **node):
        ...
        existing["updated_at"] = max(str(existing.get("updated_at") or ""), str(candidate.get("updated_at") or "")) or None
        if candidate.get("content") and len(candidate["content"]) > len(existing.get("content", "")):
    ...
    def _graph_add_source_node(self, node_map, source_type, entry, *, label, content, category=None, mood=None, updated_at=None, attributes=None):
    ...
    def _graph_add_semantic_link(self, node_map, edge_map, source_key, source_type, source_id, semantic_type, label, relation_type, *, updated_at, weight=1.0, confidence=0.8, excerpt="", attributes=None):
        entity_type, normalized_label = self.canonical_graph_entity(label, semantic_type=semantic_type)
        ...
        self.upsert_graph_node(node_map, node_key=semantic_key, node_type=entity_type, label=normalized_label, confidence=confidence, updated_at=updated_at, attributes=attributes)
        self.upsert_graph_edge(
            ...
            confidence=confidence,
            updated_at=updated_at,
            evidence=[{"source_type": source_type, "source_id": source_id, "excerpt": str(excerpt or "").strip()}],
            attributes=attributes,
        )

    def _graph_add_typed_links_from_text(self, node_map, edge_map, source_key, source_type, source_id, text, *, default_category="general", default_mood="neutral", default_day="", updated_at=None, confidence=0.75):
        for fact in self.extract_typed_graph_facts(text, source_type, default_category=default_category, default_mood=default_mood, default_day=default_day):
            self._graph_add_semantic_link(node_map, edge_map, source_key, source_type, source_id, fact["entity_type"], fact["label"], fact["relation_type"], updated_at=updated_at, weight=fact.get("weight", 1.0), confidence=max(confidence, fact.get("confidence", confidence)), excerpt=text)
    ...
    def _require_turn_temporal(self, turn_context=None):
        ...
        if temporal is None:
            ...
            runtime_wall_time = str(getattr(runtime_temporal, "wall_time", "") or "").strip()
            runtime_wall_date = str(getattr(runtime_temporal, "wall_date", "") or "").strip()
    ...
    def _project_consolidated_memories(self, node_map, edge_map, *, turn_context=None):
        ...
        for entry in self._mm.consolidated_memories():
            ...
            updated_at = str(entry.get("updated_at") or getattr(temporal, "wall_time", ""))
            category = str(entry.get("category", "general")).strip().lower() or "general"
            source_key, source_id = self._graph_add_source_node(
                node_map, "consolidated_memory", entry,
                label=summary, content=summary, category=category, mood="neutral", updated_at=updated_at,
                attributes={"summary": summary, "supporting_summaries": list(entry.get("supporting_summaries", []))[:4], "contradictions": list(entry.get("contradictions", []))[:4], "source_count": int(entry.get("source_count", 1) or 1)},
            )
            self._graph_add_semantic_link(node_map, edge_map, source_key, "consolidated_memory", source_id, "category", category, "categorized_as", updated_at=updated_at, weight=self.graph_source_weight("consolidated_memory", entry), confidence=self.graph_source_confidence("consolidated_memory", entry), excerpt=summary)
            self._graph_add_typed_links_from_text(node_map, edge_map, source_key, "consolidated_memory", source_id, summary, default_category=category, default_mood="neutral", updated_at=updated_at, confidence=self.graph_source_confidence("consolidated_memory", entry))
            for token in self.graph_keyword_tokens(summary, limit=4):
                self._graph_add_semantic_link(node_map, edge_map, source_key, "consolidated_memory", source_id, "topic", token, "mentions", updated_at=updated_at, weight=1.0, confidence=self.graph_source_confidence("consolidated_memory", entry), excerpt=summary)
            for contradiction in list(entry.get("contradictions", []))[:2]:
                ...
                _, norm_contradiction = self.canonical_graph_entity(contradiction, semantic_type=contradiction_entity_type)
                if norm_contradiction:
                    contradiction_key = self.graph_node_key(contradiction_entity_type, norm_contradiction)
                    old_edge_key = self.graph_edge_key(source_key, contradiction_key, contradiction_relation)
                ...
                self._graph_add_semantic_link(node_map, edge_map, source_key, "consolidated_memory", source_id, contradiction_entity_type, contradiction, contradiction_relation, updated_at=updated_at, weight=1.0, confidence=0.5, excerpt=contradiction)

    def _project_archive_sessions(self, node_map, edge_map, *, turn_context=None):
        temporal = self._require_turn_temporal(turn_context)
        for entry in self._mm.session_archive():
            summary = str(entry.get("summary", "")).strip()
            updated_at = str(entry.get("created_at") or getattr(temporal, "wall_time", ""))
            mood = self._bot.normalize_mood(entry.get("dominant_mood"))
            source_key, source_id = self._graph_add_source_node(
                node_map, "archive_session", entry,
                label=summary, content=summary, category="archive", mood=mood, updated_at=updated_at,
                attributes={"summary": summary, "topics": list(entry.get("topics", []))[:5], "turn_count": int(entry.get("turn_count", 0) or 0)},
            )
            for topic in entry.get("topics", [])[:5]:
                self._graph_add_semantic_link(node_map, edge_map, source_key, "archive_session", source_id, "topic", topic, "covers_topic", updated_at=updated_at, weight=1.2, confidence=self.graph_source_confidence("archive_session", entry), excerpt=summary)
            if mood != "neutral":
                self._graph_add_semantic_link(node_map, edge_map, source_key, "archive_session", source_id, "mood", mood, "reflects_mood", updated_at=updated_at, weight=1.0, confidence=self.graph_source_confidence("archive_session", entry), excerpt=summary)
            ...
            self._graph_add_typed_links_from_text(node_map, edge_map, source_key, "archive_session", source_id, summary, default_category=primary_archive_topic, default_mood=mood, updated_at=updated_at, confidence=self.graph_source_confidence("archive_session", entry))
            for token in self.graph_keyword_tokens(summary, limit=3):
                self._graph_add_semantic_link(node_map, edge_map, source_key, "archive_session", source_id, "topic", token, "mentions", updated_at=updated_at, weight=0.9, confidence=self.graph_source_confidence("archive_session", entry), excerpt=summary)
    ...
    def _project_persona_traits(self, node_map, edge_map, *, turn_context=None):
        ...
        for entry in self._mm.persona_evolution_history():
            ...
            if not trait:
                continue
            updated_at = str(entry.get("last_reinforced_at") or entry.get("applied_at") or getattr(temporal, "wall_time", ""))
            reason = str(entry.get("reason") or "").strip()
            content = " ".join(part for part in [reason, str(entry.get("announcement") or "").strip()] if part)
            source_key, source_id = self._graph_add_source_node(
                node_map, "persona_trait", entry,
                label=trait, content=content, category="persona", mood="neutral", updated_at=updated_at,
                attributes={"trait": trait, "reason": reason, "impact_score": float(entry.get("impact_score", 0.0) or 0.0), "strength": self._bot.long_term_signals.decayed_trait_strength(entry), "critique_score": int(entry.get("critique_score", 0) or 0)},
            )
            self._graph_add_semantic_link(node_map, edge_map, source_key, "persona_trait", source_id, "trait", trait, "expresses_trait", updated_at=updated_at, weight=self.graph_source_weight("persona_trait", entry), confidence=self.graph_source_confidence("persona_trait", entry), excerpt=content or trait)
            self._graph_add_typed_links_from_text(node_map, edge_map, source_key, "persona_trait", source_id, content or trait, default_category="persona", updated_at=updated_at, confidence=self.graph_source_confidence("persona_trait", entry))
            for token in self.graph_keyword_tokens(reason, limit=3):
                self._graph_add_semantic_link(node_map, edge_map, source_key, "persona_trait", source_id, "topic", token, "reinforced_by", updated_at=updated_at, weight=0.8, confidence=self.graph_source_confidence("persona_trait", entry), excerpt=reason)

    def _project_life_patterns(self, node_map, edge_map, *, turn_context=None):
        temporal = self._require_turn_temporal(turn_context)
        for entry in self._mm.life_patterns():
            summary = str(entry.get("summary", "")).strip()
            updated_at = str(entry.get("last_seen_at") or getattr(temporal, "wall_time", ""))
            ...
            source_key, source_id = self._graph_add_source_node(
                node_map, "life_pattern", entry,
                label=summary, content=str(entry.get("proactive_message") or summary).strip(), category=topic, mood=mood, updated_at=updated_at,
                attributes={"summary": summary, "topic": topic, "day_hint": day_hint, "confidence": int(entry.get("confidence", 0) or 0)},
            )
            if topic != "general":
                self._graph_add_semantic_link(node_map, edge_map, source_key, "life_pattern", source_id, "topic", topic, "pattern_topic", updated_at=updated_at, weight=1.2, confidence=self.graph_source_confidence("life_pattern", entry), excerpt=summary)
            if mood != "neutral":
                self._graph_add_semantic_link(node_map, edge_map, source_key, "life_pattern", source_id, "mood", mood, "pattern_mood", updated_at=updated_at, weight=1.0, confidence=self.graph_source_confidence("life_pattern", entry), excerpt=summary)
            if day_hint:
                self._graph_add_semantic_link(node_map, edge_map, source_key, "life_pattern", source_id, "day", day_hint, "recurs_on", updated_at=updated_at, weight=1.0, confidence=self.graph_source_confidence("life_pattern", entry), excerpt=summary)
            self._graph_add_typed_links_from_text(node_map, edge_map, source_key, "life_pattern", source_id, summary, default_category=topic, default_mood=mood, default_day=day_hint, updated_at=updated_at, confidence=self.graph_source_confidence("life_pattern", entry))
            for token in self.graph_keyword_tokens(summary, limit=3):
                self._graph_add_semantic_link(node_map, edge_map, source_key, "life_pattern", source_id, "topic", token, "mentions", updated_at=updated_at, weight=0.85, confidence=self.graph_source_confidence("life_pattern", entry), excerpt=summary)
    ...
    def build_graph_projection(self, turn_context=None):
        ...
        node_map = {}
        edge_map = {}
        self._project_consolidated_memories(node_map, edge_map, turn_context=turn_context)
        ...
        nodes = sorted(node_map.values(), key=lambda item: (item["node_type"], item["label"], item["node_key"]))
        ...
        edges = sorted(
            ...
            key=lambda item: (item["source_key"], item["relation_type"], item["target_key"]),
        )
        for edge in edges:
    ...
    def preview_memory_graph(self, snapshot=None):
        ...
        for edge in graph.get("edges", []):
            ...
            if not label:
                continue
            semantic_weights[label] = semantic_weights.get(label, 0.0) + float(edge.get("weight", 0.0) or 0.0)
        ...
        preview_nodes = []
        for label, weight in sorted(semantic_weights.items(), key=lambda item: (-item[1], item[0]))[:18]:
            node_type = semantic_types.get(label, "topic")
            preview_type = node_type if node_type in {"topic", "category", "mood"} else "topic"
            preview_nodes.append({"id": f"{preview_type}:{label}", "label": label, "type": preview_type, "weight": max(1, int(round(weight)))})

        preview_edges = [
            {"source": left, "target": right, "weight": max(1, int(round(weight)))}
            for (left, right), weight in sorted(edge_weights.items(), key=lambda item: (-item[1], item[0]))[:18]
        ]
        return {"nodes": preview_nodes, "edges": preview_edges, "updated_at": graph.get("updated_at")}
    ...
    def sync_graph_store(self, turn_context=None):
        ...
        if turn_context is not None:
            ...
            active_stage = str((getattr(turn_context, "state", None) or {}).get("_active_graph_stage") or "").strip().lower()
        ...
        try:
            ...
            self._graph_store_backend.replace_graph(snapshot.get("nodes", []), snapshot.get("edges", []))
        except Exception as exc:
        ...
        try:
            ...
            if snapshot.get("nodes") or snapshot.get("edges"):
                if not snapshot.get("updated_at"):
                    updated_at = None
                    for item in [*list(snapshot.get("nodes") or []), *list(snapshot.get("edges") or [])]:
            ...
            logger.warning("Graph store fetch failed, using in-memory projection: %s", exc)
    ...
    def graph_source_summary(self, node):
        attributes = dict(node.get("attributes") or {})
        return str(attributes.get("summary") or attributes.get("reason") or node.get("content") or node.get("label") or "").strip()
    ...
    def graph_source_lines(self, selected_items):
        ...
        for score, node, matched_labels in selected_items:
            ...
            if source_type == "consolidated_memory":
                confidence = self._bot.confidence_label(float(node.get("confidence", 0.5) or 0.5))
    ...
    def _score_graph_node(self, node, adjacency, nodes, query_tokens, query_category, query_mood, mood_trend, recent_topics):
        """Score a single source node for query relevance.
        Returns (score, matched_labels) or None if node should be excluded.
        """
        attributes = dict(node.get("attributes") or {})
        summary = self.graph_source_summary(node)
        text = " ".join(part for part in [node.get("label", ""), node.get("content", ""), summary, " ".join(attributes.get("supporting_summaries", [])), " ".join(attributes.get("contradictions", []))] if part)
        ...
        for edge in adjacency.get(node["node_key"], []):
            neighbor_key = edge.get("target_key") if edge.get("source_key") == node["node_key"] else edge.get("source_key")
            neighbor = nodes.get(neighbor_key)
            if neighbor is None or neighbor.get("node_type") in self.GRAPH_SOURCE_NODE_TYPES or neighbor.get("node_type") == "contradiction":
                continue
        ...
        confidence = max(0.4, min(1.15, float(node.get("confidence", 0.6) or 0.6) + 0.15))
        contradictions = len(attributes.get("contradictions", []))
        contradiction_penalty = max(0.5, 1.0 - contradictions * 0.12)
        score = overlap * freshness * confidence * contradiction_penalty * self._RETRIEVAL_SOURCE_TYPE_WEIGHTS.get(node.get("node_type"), 1.0)
        if score <= 0.15:
            return None
    ...
    def graph_retrieval_for_input(self, query, limit=3):
        ...
        for node in nodes.values():
            if node.get("node_type") not in self.GRAPH_SOURCE_NODE_TYPES:
                continue
            result = self._score_graph_node(node, adjacency, nodes, query_tokens, query_category, query_mood, mood_trend, recent_topics)
            if result is None:
                continue
        ...
        selected = sorted(ranked, key=lambda item: (item[0], item[1].get("updated_at", ""), item[1].get("label", "")), reverse=True)[:max(1, int(limit or 3))]
        return {
            "compressed_summary": self._graph_prompt_compressor.compress_neighborhood(
                query, snapshot.get("nodes", []), snapshot.get("edges", []),
            ...
            "supporting_evidence": [
                {
                    ...
                    "contradictions": list((node.get("attributes") or {}).get("contradictions", []))[:2],
                }
        ...
        for node in nodes.values():
            ...
            for edge in adjacency.get(node["node_key"], []):
                ...
                if neighbor.get("node_type") in self.GRAPH_SOURCE_NODE_TYPES:
                    ...
                    total_weight += float(edge.get("weight", 0.0) or 0.0) * max(0.4, float(neighbor.get("confidence", 0.0) or 0.0))
                    for peer_edge in adjacency.get(neighbor["node_key"], []):
                        peer_key = peer_edge.get("target_key") if peer_edge.get("source_key") == neighbor["node_key"] else peer_edge.get("source_key")
                        peer = nodes.get(peer_key)
                        if peer is None or peer.get("node_key") == node["node_key"] or peer.get("node_type") in self.GRAPH_SOURCE_NODE_TYPES or peer.get("node_type") == "contradiction":
                            continue
                        companion_labels.append(str(peer.get("label") or "").strip().lower())
            if not source_neighbors:
                continue
            source_types = sorted({neighbor.get("node_type") for neighbor in source_neighbors if neighbor.get("node_type")})
            ranked.append((round(total_weight, 4), node, source_types, companion_labels))
        return ranked
    ...
    def _format_graph_summary_lines(self, ranked, limit):
        ...
        lines = []
        for _score, node, source_types, companion_labels in sorted(ranked, key=lambda item: (item[0], item[1].get("label", "")), reverse=True)[:max(1, int(limit or 3))]:
            sources_text = self._bot.natural_list([source_labels.get(name, name.replace("_", " ")) for name in source_types])
            companion_unique = []
