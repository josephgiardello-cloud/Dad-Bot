...
from dadbot.utils import env_truthy, json_dumps, json_load, json_loads
...
class MemoryManager:
	...
	def consolidate_turn_outcome(self, turn_context=None, result=None):
		...
		try:
			...
			logger.warning("MemoryManager.consolidate_turn_outcome failed (non-fatal): %s", exc)
	...
	def store_cached_embeddings(self, model_name, text_to_embedding):
		return self._semantic_manager.store_cached_embeddings(model_name, text_to_embedding)
	...
	def semantic_memory_filters(query_tokens, query_category, query_mood):
		return SemanticIndexManager.semantic_memory_filters(query_tokens, query_category, query_mood)
	...
	def filtered_semantic_rows(self, where_clauses, params, candidate_limit):
		return self._semantic_manager.filtered_semantic_rows(where_clauses, params, candidate_limit)
	...
	def semantic_candidate_rows(self, where_clauses, params, candidate_limit, query_embedding=None, query_tokens=None, query_category="general", query_mood="neutral"):
		return self._semantic_manager.semantic_candidate_rows(where_clauses, params, candidate_limit, query_embedding=query_embedding, query_tokens=query_tokens, query_category=query_category, query_mood=query_mood)

	def score_semantic_rows(self, rows, current_memories, query_embedding):
		return self._semantic_manager.score_semantic_rows(rows, current_memories, query_embedding)
	...
	def semantic_memory_matches(self, query, memories, limit=3):
		...
		for similarity, memory in self.score_semantic_rows(rows, current_memories, query_context["query_embedding"]):
			...
			impact_bonus = min(1.5, max(0.0, self.bot.memory_impact_score(memory)) * 0.35)
			if score > 0:
				score += impact_bonus
			if score > 0.1:
	...
	def canonical_graph_entity(self, label, semantic_type=None):
		return self._graph_manager.canonical_graph_entity(label, semantic_type=semantic_type)
	...
	def relation_type_for_entity(self, source_type, entity_type, text, fallback="mentions"):
		return self._graph_manager.relation_type_for_entity(source_type, entity_type, text, fallback=fallback)

	def extract_typed_graph_facts(self, text, source_type, *, default_category="general", default_mood="neutral", default_day=""):
		return self._graph_manager.extract_typed_graph_facts(text, source_type, default_category=default_category, default_mood=default_mood, default_day=default_day)
