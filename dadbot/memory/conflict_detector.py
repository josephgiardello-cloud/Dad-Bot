...
class ConflictDetector:
    ...
    def contradiction_signal_reason(
        ...
        stripped_left = re.sub(
            r"\b(?:not|never|no|doesn't|dont|don't|isn't|wasn't|can't|cannot|won't|stopped|quit)\b",
            "",
            left,
        )
        ...
        shared_tokens = self.bot.significant_tokens(
            stripped_left,
        ) & self.bot.significant_tokens(stripped_right)
        negation_words = {
            "not",
            "never",
            "no",
                ...
                candidate_entries.append(
                    {
                        "summary": summary,
                        "category": str(
                            item.get("category") or self.bot.infer_memory_category(summary),
                        )
                        .strip()
                        .lower()
                        or "general",
                        ...
                        "updated_at": str(
                            item.get("updated_at") or item.get("created_at") or date.today().isoformat(),
                        ),
                    },
                )
                ...
                candidate_entries.append(
                    {
                        ...
                        "confidence": float(item.get("confidence", 0.5) or 0.5),
                        "updated_at": str(
                            item.get("updated_at") or date.today().isoformat(),
                        ),
                    },
                )
    ...
    def consolidated_contradictions(self, limit: int = 8) -> list:
                ...
                findings.append(
                    {
                        ...
                        "left_importance": float(
                            left.get("importance_score", 0.0) or 0.0,
                        ),
                        "right_importance": float(
                            right.get("importance_score", 0.0) or 0.0,
                        ),
        ...
        findings.sort(
            key=lambda item: (
                max(
                    float(item.get("left_importance", 0.0) or 0.0),
                    float(item.get("right_importance", 0.0) or 0.0),
                ),
                max(
                    float(item.get("left_confidence", 0.0) or 0.0),
                    float(item.get("right_confidence", 0.0) or 0.0),
                ),
    ...
    def resolve_consolidated_contradiction(
        ...
        winner["confidence"] = round(
            min(0.98, float(winner.get("confidence", 0.5) or 0.5) + 0.04),
            3,
        )
        winner["importance_score"] = round(
            min(1.0, float(winner.get("importance_score", 0.0) or 0.0) + 0.06),
            3,
        )
    ...
    def _entry_identity(entry: dict) -> str:
        ...
        return hashlib.sha1(f"{category}|{summary}".encode()).hexdigest()[:12]
