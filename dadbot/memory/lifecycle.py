...
class MemoryLifecycleManager:
    ...
    def persona_evolution_history(self):
        history = self.bot.runtime_config.tail(
            [
                entry
                for entry in (
                    self._manager.normalize_persona_evolution_entry(item)
                    for item in self.bot.MEMORY_STORE.get("persona_evolution", [])
                )
                if entry is not None
            ],
            "persona_evolution",
    ...
    def wisdom_catalog(self):
        insights = self.bot.runtime_config.tail(
            [
                entry
                for entry in (
                    self._manager.normalize_wisdom_entry(item)
                    for item in self.bot.MEMORY_STORE.get("wisdom_insights", [])
                )
                if entry is not None
            ],
            "wisdom_insights",
    ...
    def life_patterns(self):
        patterns = self.bot.runtime_config.tail(
            [
                entry
                for entry in (
                    self._manager.normalize_life_pattern_entry(item)
                    for item in self.bot.MEMORY_STORE.get("life_patterns", [])
                )
                if entry is not None
            ],
            "life_patterns",
    ...
    def pending_proactive_messages(self):
        messages = self.bot.runtime_config.tail(
            [
                entry
                for entry in (
                    self._manager.normalize_proactive_message_entry(item)
                    for item in self.bot.MEMORY_STORE.get(
    ...
    def consolidated_memories(self):
        consolidated = self.bot.runtime_config.tail(
            [
                entry
                for entry in (
                    self._manager.normalize_consolidated_memory_entry(item)
    ...
    def memory_graph_snapshot(self):
        graph = self._manager.normalize_memory_graph(
            self.bot.MEMORY_STORE.get("memory_graph"),
        )
    ...
    def queue_proactive_message(self, message, source="general"):
        ...
        if any(
            self.bot.normalize_memory_text(item.get("message", "")) == self.bot.normalize_memory_text(entry["message"])
            for item in queued[-dedup_window:]
        ):
            return None

        queued.append(entry)
        self._manager.mutate_memory_store(
            pending_proactive_messages=self.bot.runtime_config.tail(
                queued,
                "pending_proactive_messages",
            ),
        )
