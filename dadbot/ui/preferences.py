...
def _render_col1_display_voice(preferences: dict, voice: dict) -> dict:
    ...
    return {
        ...
        "auto_mood_theme": auto_mood_theme,
...
def _render_col2_llm_tools_audio(
    ...
    """Render column 2 (LLM, tools, notifications, audio) widgets; return collected values."""
    ...
    if llm_provider == "ollama":
        st.caption("Local Ollama â€” no API key required.")
    else:
    ...
    return {
        ...
        "auto_web_lookup": auto_web_lookup,
        ...
        "max_thinking_time_seconds": max_thinking_time_seconds,
        "stream_max_chars": stream_max_chars,
        "stt_enabled": stt_enabled,
        "stt_backend": stt_backend,
        "stt_model": stt_model,
        "tts_enabled": tts_enabled,
        "tts_backend_pref": tts_backend_pref,
        "tts_piper_model_path": tts_piper_model_path,
        "tts_voice": tts_voice,
        "tts_autoplay": tts_autoplay,
        "tts_rate": tts_rate,
        "warmth": warmth,
        "dad_joke_frequency": dad_joke_frequency,
        "pacing": pacing,
    }
...
def _save_preferences(
    ...
    family_echo_turn_interval: int,
    wisdom_turn_interval: int,
    life_pattern_queue_limit: int,
    calibration_enabled: bool,
    opening_line: str,
) -> None:
    """Persist all collected form values to bot profile."""
    preferences.update(
        {
            "theme_mode": c1["theme_mode"],
            "auto_mood_theme": bool(c1["auto_mood_theme"]),
            "power_mode": c1["power_mode"],
            "font_scale": c1["font_scale"],
    ...
    updated_voice = {
        "enabled": bool(c1["voice_enabled"]),
        "mode": c1["voice_mode"],
        "auto_send_always_listening": bool(c1["auto_send"]),
        "wake_word_required": bool(c1["wake_word_required"]),
        "wake_word_phrase": str(c1["wake_word_phrase"] or "hey dad").strip().lower() or "hey dad",
        "stt_enabled": bool(c2["stt_enabled"]),
        "stt_backend": c2["stt_backend"],
        "stt_model": c2["stt_model"],
        "stt_language": str(voice.get("stt_language") or "en"),
        "tts_enabled": bool(c2["tts_enabled"]),
        "tts_autoplay": bool(c2["tts_autoplay"]),
    ...
    bot.PROFILE["llm"] = {
        "provider": str(c2["llm_provider"] or "ollama").strip().lower() or "ollama",
        "model": str(c2["llm_model"] or bot.MODEL_NAME).strip() or bot.MODEL_NAME,
    }
    ...
    bot.PROFILE["notifications"] = {
        ...
        "quiet_hours_start": int(c2["quiet_hours_start"]),
        "quiet_hours_end": int(c2["quiet_hours_end"]),
        "notify_patterns": bool(c2["notify_patterns"]),
        "notify_reminders": bool(c2["notify_reminders"]),
    }
    bot.save_profile()
