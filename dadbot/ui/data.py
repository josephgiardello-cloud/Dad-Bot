...
def render_memory_garden(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "A calmer view of Dad's memories. Search by phrase, then browse like a notebook.",
        )
        all_memories = bot.memory_catalog()
        memory_categories = sorted(
            set(str(item.get("category") or "general") for item in all_memories),
        )
        garden_col1, garden_col2, garden_col3 = st.columns([3, 2, 1])
        with garden_col1:
            garden_search = st.text_input(
                "Search memories",
        ...
        with garden_col2:
            garden_category = st.selectbox(
                "Category",
                options=["all"] + memory_categories,
                key="garden-category",
                label_visibility="collapsed",
            )
        with garden_col3:
            chapters_view = st.checkbox(
                "Chapters",
                value=False,
                key="garden-chapters",
                help="Group memories by Life Chapter (category)",
            )

        filtered_memories = filter_memory_entries(
            all_memories,
            garden_search,
            garden_category,
        )
        display_limit = 30
        st.caption(
            f"Showing {min(len(filtered_memories), display_limit)} of {len(all_memories)} memory notes",
        )
        ...
        def _render_memory_card(memory_entry, card_key_suffix):
            ...
            memory_importance = float(
                memory_entry.get("importance_score") or memory_entry.get("importance", 0.5) or 0.5,
            )
            memory_date_raw = str(
                memory_entry.get("created_at") or memory_entry.get("updated_at") or "",
            )[:10]
            try:
                memory_age_days = (
                    (_date_cls.today() - _date_cls.fromisoformat(memory_date_raw)).days if memory_date_raw else 0
                )
            except Exception:
                memory_age_days = 0
            ...
            with st.container(border=True):
                ...
                if mc4.button(
                    star_label,
                    key=f"pin-{card_key_suffix}-{hash(memory_summary)}",
        ...
        if chapters_view:
            ...
            for memory_entry in filtered_memories[:display_limit]:
                chapter_groups[str(memory_entry.get("category") or "general")].append(
                    memory_entry,
                )
            for chapter_category in sorted(chapter_groups.keys()):
                ...
                with st.expander(chapter_title, expanded=False):
                    for chapter_idx, memory_entry in enumerate(chapter_memories):
                        _render_memory_card(
                            memory_entry,
                            f"chap-{chapter_category}-{chapter_idx}",
                        )
        else:
            for memory_idx, memory_entry in enumerate(
                filtered_memories[:display_limit],
            ):
                _render_memory_card(memory_entry, str(memory_idx))

        if len(filtered_memories) > display_limit:
            st.caption(
                f"...and {len(filtered_memories) - display_limit} more. Refine your search to see specific entries.",
            )
...
def _render_calendar_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Connect a public iCal/ICS feed (Google Calendar, Apple Calendar, Outlook) so Dad sees your upcoming events.",
        )
        _ical_url = str(
            (bot.PROFILE.get("ical_feed_url") if isinstance(bot.PROFILE, dict) else None) or "",
        ).strip()
        ...
        if _ical_save_col.button("Save feed URL", use_container_width=True):
            if isinstance(bot.PROFILE, dict):
                ...
                st.rerun()
        if _ical_fetch_col.button(
            "Preview upcoming events",
            use_container_width=True,
            disabled=not str(_ical_input or "").strip(),
        ):
            with st.spinner("Fetching calendar feed..."):
                _events, _err = fetch_ical_events(
                    str(_ical_input).strip(),
                    max_events=12,
                )
            if _err:
                st.error(_err)
                ...
                for _ev in _events:
                    ...
                    st.markdown(
                        f"**{_ev_date}{'â€“' + _ev_end if _ev_end and _ev_end != _ev_date else ''}** â€” {_ev['summary']}{_ev_loc}{_ev_desc}",
                    )
        if _ical_url:
            st.caption(
                f"Configured feed: `{_ical_url[:80]}{'...' if len(_ical_url) > 80 else ''}`",
            )
            st.caption(
                "Dad will be aware of your upcoming events when this feed is set.",
            )
...
def _render_heritage_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Upload journals, exports, and photo metadata so Dad can bootstrap long-term memory from your history.",
        )
        ...
        max_items = st.slider(
            "Max extracted memories per file",
            min_value=5,
            max_value=120,
            value=40,
            step=5,
        ...
        if preview_col.button(
            "Preview heritage import",
            use_container_width=True,
            disabled=not _validated_uploads,
        ):
        ...
        if preview_memories:
            ...
            for entry in preview_memories[:10]:
                st.markdown(
                    f"- [{entry.get('category', 'heritage')}] {entry.get('summary', '')}",
                )
            if len(preview_memories) > 10:
                st.caption(f"...and {len(preview_memories) - 10} more")
        if import_col.button(
            "Import into Dad memory",
            use_container_width=True,
            type="primary",
            disabled=not preview_memories,
        ):
            added = 0
...
def _render_download_section(
    ...
    with st.container(border=True):
        ...
        st.caption(
            "Pull out the current memory store, profile, or a combined support bundle.",
        )
        download_col1, download_col2, download_col3 = st.columns(3)
        download_col1.download_button(
            "Download memory JSON",
            data=memory_payload,
            file_name="dad_memory_export.json",
        ...
        download_col2.download_button(
            ...
            file_name="dad_profile_export.json",
            mime="application/json",
            use_container_width=True,
        )
        download_col3.download_button(
            ...
            data=bundle_json,
            file_name=f"dadbot_bundle_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            mime="application/json",
            use_container_width=True,
        )
        path_col1, path_col2 = st.columns([3, 1])
        export_path = path_col1.text_input(
            "Export memory to workspace path",
            value=st.session_state.export_path,
        )
...
def _render_memory_management_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        if st.button("Delete selected memory", disabled=not selected_memory):
            ...
            if removed:
                st.success(
                    f"Removed {len(removed)} memory entry."
                    if len(removed) == 1
                    else f"Removed {len(removed)} memory entries.",
                )
                st.rerun()
        ...
        if st.button("Run controlled forgetting", use_container_width=True):
            ...
            if removed > 0:
                st.success(
                    f"Forgot {removed} low-importance memories. Backup: {result.get('backup_path', '')}",
                )
                st.rerun()
...
def _render_consolidated_feedback_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Upvote or downvote these to reinforce what Dad should treat as durable truth.",
        )
...
def _render_conflict_resolution_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Memories where Dad holds conflicting beliefs. Pick which one is true to resolve the contradiction.",
        )
        ...
        if not _contradictions:
            ...
            for _ci, _pair in enumerate(_contradictions):
                ...
                with st.container(border=True):
                    ...
                    if st.button(
                        "Resolve",
                        key=f"resolve-btn-{_ci}-{hash(_left)}",
                        use_container_width=False,
                    ):
                        if _resolution == "Keep A":
                            bot.resolve_consolidated_contradiction(
                                _left,
                                _right,
                                keep="left",
                                reason="user_review",
...
def _render_living_snapshot_section(living: dict) -> None:
    with st.container(border=True):
        ...
        with _snap_cols[1]:
            _patterns = [
                entry.get("pattern", entry.get("summary", ""))
                for entry in living["patterns"]
                if entry.get("pattern") or entry.get("summary")
            ]
...
def _render_journal_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Weekly reflections from Dad's point of view, written as short journal pages.",
        )
        ...
        _existing_this_week = next(
            (e for e in _journal_entries if e.get("week") == _this_week),
            None,
        )
        _journal_col1, _journal_col2 = st.columns([3, 1])
        with _journal_col2:
            _force_regen = st.button(
        ...
        if _force_regen or (_existing_this_week is None and len(bot.session_archive()) > 0):
            ...
            with st.spinner("Dad is writing in his journal..."):
                try:
                    ...
                    _journal_text = (
                        bot.extract_ollama_message_content(_journal_resp)
                        or "(Dad hasn't written this week's entry yet.)"
                    )
                    _new_entry = {
                        "week": _this_week,
        ...
        if _journal_entries:
            for _jentry in reversed(_journal_entries[-8:]):
                ...
                with st.expander(
                    f"Week {_jwk}  ({_jdate})",
                    expanded=_jwk == _this_week,
                ):
                    st.markdown(_jentry.get("text", "_(empty)_"))
        else:
            st.caption(
                "No journal entries yet. Start chatting and click 'Write this week's entry' to generate one.",
            )
...
def _render_knowledge_web_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Visual map of what Dad has learned from long-term memory and relationship patterns.",
        )
        ...
        if not nodes:
            st.caption(
                "No graph nodes yet. Keep chatting and run periodic synthesis to build this map.",
            )
        else:
            dot_lines = [
                "digraph DadKnowledge {",
                "rankdir=LR;",
                "node [shape=ellipse, style=filled, fillcolor=lightgoldenrod1, color=gray40];",
            ]
            for node in nodes:
...
def _render_memory_search_section(bot: DadBot) -> None:
    with st.container(border=True):
        ...
        st.caption(
            "Inspect what memories Dad can retrieve for a topic before asking for advice.",
        )
        query = st.text_input(
            "Search memories",
            value="",
            placeholder="e.g., work stress, sleep, routines",
        )
        ...
        if st.button(
            "Search",
            use_container_width=True,
            disabled=not str(query or "").strip(),
        ):
            retrieval = bot.retrieve_context(
                str(query).strip(),
                strategy=strategy,
                limit=6,
            )
